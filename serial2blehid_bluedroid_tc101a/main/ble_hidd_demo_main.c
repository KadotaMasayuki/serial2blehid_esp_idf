...
// ペアリングしたときのデバイス名。63文字＋終端ヌル文字　までっぽい
#define HIDD_DEVICE_NAME "SERIAL_HID_250819_01"
...
/**
 * @brief ペアリングできていれば、指定された文字列を１文字ずつHID-BLEのキーコードに変換して送信
 *        ただし、キーボードのキートップに印字されている文字に限る。
 *        また、F1キーなどの、押しても文字として見えないキーは無視する。
 *        具体的には、tab(0x09), lf(0x0a), cr(0x0d) と、スペース(0x20)からチルダ(0x7e)まで。それ以外の文字は送信しない。
 *        指定された以外の追加の文字は送信しない。終端文字が必要であれば、終端文字を付けたデータを指定すること。
 * @param *data ascii文字列
 * @param len ascii文字列の長さ
 * @return １文字でも送信できればtrue
 */
bool send_ble_hid_by_string(uint8_t *data, int len) {
  int cnt = 0;
  if (sec_conn) {
    uint8_t k;
    key_mask_t m;
    for (int i = 0; i < len; i ++) {
      ...
      if (k != 0) {
        ...
        // 停止する（修飾キー無しの長さゼロのキーストロークを送る）
        esp_hidd_send_keyboard_value(hid_conn_id, 0, &k, 0);
        vTaskDelay(50 / portTICK_PERIOD_MS);
        cnt ++;
      }
    }
...
/**
 * @brief ペアリングできていれば、指定されたHID-BLEのキーコードとマスクを送信
 *        ただし、send_ble_hid_by_string()で送出できる文字に限る
 * @param k 送出するスキャンコード
 * @param m マスク（シフトキーとか、コントロールキーとか）
 * @return １文字でも送信できればtrue
 */
bool send_ble_hid_by_keycode(uint8_t k, key_mask_t m) {
  if (sec_conn) {
    // 1文字送る
    esp_hidd_send_keyboard_value(hid_conn_id, m, &k, 1);
    vTaskDelay(50 / portTICK_PERIOD_MS);
