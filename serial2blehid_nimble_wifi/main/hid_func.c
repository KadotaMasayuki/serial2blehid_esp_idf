...
/* notify data buffers */
static uint8_t
    /* mouse: byte 0: bit 0 Button 1, bit 1 Button 2, bit 2 Button 3, bits 4 to
    7 zero byte 1 : X displacement, byte 2: Y displacement, byte 3: wheel    */
    ...
    byte 0: bits 0-3 num key pad, 4-5 - channel (+1 -1), 6 - volume up, 7 -
    volume down byte 1: 0-3 - buttons, 4-5 - selection buttons, 6-7 - zero    */
    CC_buffer[HIDD_LE_REPORT_CC_SIZE],
    /* Keyboard out report keeps data for leds in one byte
    LEDS: bit 0 NUM LOCK, 1 CAPS LOCK, 2 SCROLL LOCK, 3 COMPOSE, 4 KANA, 5 to 7
    RESERVED (zeroes) */
...
/* mark report for indicate/notify when central subscribes to service
 * charachetric with report */
void hid_set_notify(uint16_t attr_handle, uint8_t cur_notify,
                    uint8_t cur_indicate) {
    int report_idx = -1;

    /* find hid_notify_data struct index of reports array for given atribute
     * handle */
    for (int i = 0;
         i < sizeof(Notify_data_reports) / sizeof(Notify_data_reports[0]);
         ++i) {
        uint16_t current_handle_idx =
            My_hid_dev.report_mode_boot ? Notify_data_reports[i].handle_boot_num
    ...
    if (report_idx == -1) {
        ESP_LOGW(tag, "%s: attr_handle %04X not found in reports", __FUNCTION__,
                 attr_handle);
    } else {
        ...
        ESP_LOGI(tag, "%s: service %s, attr_handle %d, notify %d, indicate %d",
                 __FUNCTION__, Notify_data_reports[report_idx].name,
                 attr_handle, cur_notify, cur_indicate);
    }
}
...
static int lock_hid_data() {
    ...
    if (xSemaphoreTake(My_hid_dev.semaphore,
                       pdMS_TO_TICKS(HID_DEV_BUF_MUTEX_WAIT)) == pdTRUE) {
        return 0;
    } else {
    ...
    for (int i = 0;
        ...
        switch (Notify_data_reports[i].handle_num) {
            ...
            case HANDLE_HID_CC_REPORT:
                memset(Notify_data_reports[i].buffer, 0,
                       Notify_data_reports[i].buffer_size);
        }
    }
...
void hid_set_disconnected() { My_hid_dev.connected = false; }
...
int hid_read_buffer(struct os_mbuf *buf, int handle_num) {
    ...
    if (rep_idx != -1 && lock_hid_data() == 0) {
        rc = os_mbuf_append(buf, Notify_data_reports[rep_idx].buffer,
        ...
        if (rep_idx == -1)
            ESP_LOGW(tag, "%s: handle_num %d not found", __FUNCTION__,
                     handle_num);
        return 2;
    }
...
int hid_write_buffer(struct os_mbuf *buf, int handle_num) {
    ...
    if (rep_idx != -1 && lock_hid_data() == 0) {
        if (OS_MBUF_PKTLEN(buf) == Notify_data_reports[rep_idx].buffer_size) {
            rc = ble_hs_mbuf_to_flat(buf, Notify_data_reports[rep_idx].buffer,
                                     OS_MBUF_PKTLEN(buf), NULL);
        } else {
            rc = 4;
        }
        unlock_hid_data();
        if (rc == 0) {
            if (handle_num == HANDLE_HID_KB_OUT_REPORT) {
                ...
                my_if_uart_set_leds(Leds_buffer[0]);
            }
        }
    } else {
        return 2;
...
int hid_send_report(int report_handle_num) {
    ...
    if (!My_hid_dev.semaphore || !My_hid_dev.connected ||
        My_hid_dev.suspended_state) {
        ESP_LOGI(tag, "%s semaphore %p %d %d", __FUNCTION__,
                 My_hid_dev.semaphore, My_hid_dev.connected,
                 My_hid_dev.suspended_state);
        return 1;
    }
    ...
    if (report_idx == -1) {
        ESP_LOGW(tag, "%s: Unknown report_handle_num %d", __FUNCTION__,
                 report_handle_num);
        return 2;
    }
    ...
    if (My_hid_dev.report_mode_boot) {
        send_handle =
            Svc_char_handles[Notify_data_reports[report_idx].handle_boot_num];
    } else {
        send_handle =
            Svc_char_handles[Notify_data_reports[report_idx].handle_num];
    }
        ...
        case SEND_METHOD_CUSTOM: {
            if (lock_hid_data() == 0) {
                ...
                if (Notify_data_reports[report_idx].can_indicate) {
                    rc = ble_gattc_indicate_custom(My_hid_dev.conn_handle,
                                                   send_handle, om);
                } else if (Notify_data_reports[report_idx].can_notify) {
                    rc = ble_gattc_notify_custom(My_hid_dev.conn_handle,
                                                 send_handle, om);
                }
            }
            break;
...
uint8_t hid_battery_level_get(void) { return Battery_level[0]; }
...
int hid_mouse_change_key(int cmd, int8_t move_x, int8_t move_y, bool pressed) {
    int rc = 0;
...
int hid_cc_build_report(uint8_t *buffer, consumer_cmd_t cmd, bool pressed) {
    ...
    switch (cmd) {
        ...
        case HID_CONSUMER_CHANNEL_DOWN:
            HID_CC_RPT_SET_CHANNEL(buffer,
                                   pressed ? HID_CC_RPT_CHANNEL_DOWN : 0);
            break;
        ...
        case HID_CONSUMER_SCAN_NEXT_TRK:
            HID_CC_RPT_SET_BUTTON(buffer,
                                  pressed ? HID_CC_RPT_SCAN_NEXT_TRK : 0);
            break;

        case HID_CONSUMER_SCAN_PREV_TRK:
            HID_CC_RPT_SET_BUTTON(buffer,
                                  pressed ? HID_CC_RPT_SCAN_PREV_TRK : 0);
            break;
...
int hid_cc_change_key(int key, bool pressed) {
    int rc = 0;
...
/**
 * @brief process one key stroke
 * @param key hid scan code include modifier key
 * @param pressed 1:press, 0:release
 */
int hid_keyboard_change_key(uint8_t key, bool pressed) {
    int rc = 0;
    ...
    if (lock_hid_data() == 0) {
        ...
        if (rc == 0) {
            // show report
            char hex_string[HIDD_LE_REPORT_KB_IN_SIZE * 5 + 1];
            int offset = 0;
            for (int i = 0; i < HIDD_LE_REPORT_KB_IN_SIZE; i++) {
                offset +=
                    sprintf(hex_string + offset, "0x%02x ", Keyboard_buffer[i]);
            }
            ESP_LOGI(tag, "report -> %s", hex_string);
            // send report
...
/**
 * @brief process multi key combinations
 * @m modifier key scan code. 0 means no modifier key.
 * @param keys[] array of key hid scan code. 0 means no key.
 * @param pressed 1:press, 0:release
 */
int hid_keyboard_change_keycombination_multi(
    uint8_t m, uint8_t keys[HIDD_LE_REPORT_KB_IN_SIZE - 2], bool pressed) {
    int rc = 0;
    ...
    if (lock_hid_data() == 0) {
        ...
        if (pressed) {
            ...
            // modifier key
            if (m > 0) {
                Keyboard_buffer[0] = 1 << (m - HID_KEY_LEFT_CTRL);
            } else {
                Keyboard_buffer[0] = 0;
            }
            // if pressed, adding key to buffer
            for (int i = 2; i < HIDD_LE_REPORT_KB_IN_SIZE; ++i) {
                Keyboard_buffer[i] = keys[i - 2];
            }
        } else {
            // released
            // modifier key
            Keyboard_buffer[0] = 0;
            // if released, delete (shift other keys to left)
            for (int i = 2; i < HIDD_LE_REPORT_KB_IN_SIZE; ++i) {
                Keyboard_buffer[i] = 0;
            }
        }
...
/**
 * @brief process single key combination
 * @m modifier key scan code. 0 means no modifier key.
 * @param single key hid scan code. 0 means no key.
 * @param pressed 1:press, 0:release
 */
int hid_keyboard_change_keycombination_single(uint8_t m, uint8_t key,
                                              bool pressed) {
    int rc = 0;
    uint8_t keys[HIDD_LE_REPORT_KB_IN_SIZE - 2];
    keys[0] = key;
    for (int i = 1; i < HIDD_LE_REPORT_KB_IN_SIZE - 2; i++) {
        keys[i] = 0;
    }
    rc = hid_keyboard_change_keycombination_multi(m, keys, pressed);
    return rc;
}
